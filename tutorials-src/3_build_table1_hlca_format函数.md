...
把这段脚本重构为 `jwtools` 包里的一个标准函数，遵循 R 包开发规范：

1. **函数名**：`build_table1_hlca_format()` —— 动词开头，符合 tidyverse 风格
2. **参数化**：原脚本里硬编码的白名单列、`sample`名解析正则、输出路径等全部变成可配置参数，保持默认值与你当前项目一致，但其他项目（HeartERVmap/PH-ERVAtlas等）也能复用
3. **不强制写文件**：默认只返回结果列表（in-memory），`save_output=TRUE`时才写盘，符合包函数"纯函数优先"的设计原则
4. **显式命名空间**：包内函数用 `pkg::fun()` 写法，避免 `library()` 依赖冲突（这是CRAN/R包开发的硬性要求）
5. **roxygen2 文档**：可以直接 `devtools::document()` 生成帮助文档
...
```r
# 1. 把上面两个函数存为 jwtools 包内的文件
usethis::use_r("build_table1_hlca_format")   # 会打开对应的 R/ 文件

# 2. 确保 DESCRIPTION 里声明了依赖(若尚未声明)
...
# 4. 本地测试
devtools::load_all()
res <- build_table1_hlca_format(immune.combined, save_output = TRUE, output_dir = "results")
res$table1
res$na_report
```
...
| 改进点 | 说明 |
|---|---|
| **可参数化的白名单** | `sample_level_cols` 现在是参数而非硬编码，其他项目（HeartERVmap等）字段体系不同时可直接传入自定义白名单 |
| **可插拔的subject_ID解析器** | `subject_id_parser` 参数允许传入自定义函数，不再绑死 `Hs_WT_*`/`Hs_Banovich_*` 这两种命名规则 |
| **默认不写文件** | 包函数默认返回 in-memory 结果（`save_output=FALSE`），避免调用一次函数就在用户工作目录里产生副作用文件，这是R包开发的标准实践 |
| **返回值结构化** | 返回一个list（`table1`/`sample_summary`/`na_report`/`excluded_cols`/`n_cells_check`），下游可以直接 `res$sample_summary` 喂给你的 `z5_33` 合并脚本，不需要重新跑一次折叠 |
| **命名空间显式化** | 全部用 `dplyr::`、`stringr::`、`openxlsx::` 前缀，符合R包CRAN检查要求，避免与主脚本里 `library()` 加载的其他包函数冲突 |

需要我把 `z5_33`（合并`table1_hlca_format`与`table1_final_v3`那部分）也一并重构成 `jwtools` 里的配套函数（比如 `merge_table1_sources()`）吗？
